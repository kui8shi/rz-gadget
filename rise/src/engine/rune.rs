...
impl<S, Exp, Ctx> RzSym<Ctx, Exp, S>
where Ctx: Context<IFn=qf_abv::QF_ABV_Fn>,
      Exp: PathExplorer<Ctrl = Control, Ctx = Ctx>,
      S: InstructionStream<Output = LOpInfo, Index = u64>
{

    pub fn new(ctx: Ctx, exp: Exp, stream: S) -> RzSym<Ctx, Exp, S> {
        RzSym {
            ctx,
            explorer: exp,
        ...
        let read = match *t.unwrap() {
            Token::ERegister(ref name) | Token::EIdentifier(ref name) => {
                if self.ctx.alias_of(name.clone()) == Some("PC".to_owned()) {
                    let ip = self.ctx.ip();
                    self.ctx.define_const_bv(ip, 64)
                } else {
                    self.ctx.reg_read(name)
                }
            }
            Token::EEntry(ref id, Some(64)) => self.intermediates[*id].clone(),
            Token::EConstant(value) => self.ctx.define_const_bv(value, 64),
            Token::EAddress => {
                let ip = self.ctx.ip();
                self.ctx.define_const_bv(ip, 64)
            }
            ...
            Token::ELastsz => self.ctx.define_const_bv(64, 64),
            _ => unreachable!(),
        };
        Ok(Some(read))
    }
        ...
        *control = Control::Continue;
        ...
        let result = match token {
            ...
            Token::ECmp | Token::ELt | Token::EGt => {
                ...
                let const_0 = self.ctx.define_const_bv(0, 64);
                let const_1 = self.ctx.define_const_bv(1, 64);
...
impl<Ctx, Exp, S> Engine for RzSym<Ctx, Exp, S>
where Ctx: Context<IFn=qf_abv::QF_ABV_Fn>,
      Exp: PathExplorer<Ctrl = Control, Ctx = Ctx>,
      S: InstructionStream<Output = LOpInfo, Index = u64>
{
