...
where Ctx: Context {
    ...
    fn next(&mut self, _: &mut Self::Ctx) -> Control {
    ...
    fn next_job(&mut self, ctx: &mut Self::Ctx) -> Option<Control> {
        ...
        if let Some(ref state) = self.queue.pop_front() {
            *ctx = state.ctx.clone();
            Some(
                match state.branch {
                    BranchType::True => Control::ExploreTrue,
                    BranchType::False => Control::ExploreFalse,
                })
        } else {
            None
        }
    ...
    fn register_branch(&mut self, ctx: &mut Self::Ctx, _condition: <Self::Ctx as RegisterRead>::VarRef) -> Control {
